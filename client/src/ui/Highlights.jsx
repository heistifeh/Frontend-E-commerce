    ...
    <Container className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
      {
        highlightsData.map((item)=>(
          <div key={item?.id} className='relative h-60 rounded-lg shadow-md cursor-pointer overflow-hidden group'>
            ...
            <div className='relative z-10 p-6 flex flex-col justify-between h-full' style={{ color: item?.color}}>
              ...
              <Link to={item?._base}
              className='text-base font-normal'>{item?.buttonTitle}</Link>
            </div>
